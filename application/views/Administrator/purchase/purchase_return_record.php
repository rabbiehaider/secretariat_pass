					...
					<form class="form-inline" id="searchForm" @submit.prevent="getPurchaseReturns">
						<div class="form-group">
							...
							<v-select v-bind:options="suppliers" v-model="selectedSupplier"
								label="display_name" placeholder="Select Supplier"></v-select>
						</div>
					...
					<tbody>
						<tr v-for="pr in returns">
							...
							<td style="text-align:center;">
								<a href="" title="Return Invoice"
									v-bind:href="`/purchase_return_invoice/${pr.PurchaseReturn_SlNo}`"
									target="_blank"><i class="fa fa-file"></i></a>
								<?php if ($this->session->userdata('accountType') != 'u') { ?>
									<a href="" title="Edit" v-bind:href="`purchaseReturns/${pr.PurchaseReturn_SlNo}`"><i
											class="fa fa-edit"></i></a>
									<a href="" title="Delete Return"
										@click.prevent="deletePurchaseReturn(pr.PurchaseReturn_SlNo)"><i
						...
						<tr style="font-weight:bold;">
							...
							<td style="text-align:right;">{{ returns.reduce((prev, curr) => { return prev +
								parseFloat(curr.PurchaseReturn_ReturnAmount)}, 0) | decimal }}</td>
							<td></td>
						</tr>
					</tbody>
				</table>
