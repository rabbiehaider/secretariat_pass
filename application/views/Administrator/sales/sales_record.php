...
<script>
	...
	new Vue({
		...
		methods: {
			...
			getSalesRecord() {
				let filter = {
					userId: this.selectedUser?.User_SlNo ?? '',
			...
			getSaleDetails() {
				let filter = {
					categoryId: this.selectedCategory?.Category_SlNo ?? '',
					productId: this.selectedProduct?.Product_SlNo ?? '',
					dateFrom: this.dateFrom,
					dateTo: this.dateTo
				}

				axios.post('/get_saledetails', filter).then(res => {
					let sales = res.data;

					if (this.selectedProduct == null) {
						sales = _.chain(sales).groupBy('ProductCategory_ID').map(sale => {
							return {
								category_name: sale[0].Category_Name,
								products: _.chain(sale).groupBy('Product_IDNo').map(product => {
									return {
			...
			async print() {
				...
				let categoryText = '';
				if (this.selectedCategory != null && this.selectedCategory.Category_SlNo != '' && this.searchType == 'category') {
					categoryText = `<strong>Category: </strong> ${this.selectedCategory.Category_Name}`;
				}
