...
class Transfer extends CI_Controller
{
    ...
    public function addProductTransfer()
    {
        ...
        try {
            ...
            $this->db->insert('tbl_transfer_master', $transfer);
            ...
            foreach ($data->cart as $cartProduct) {
                ...
                $this->db->insert('tbl_transfer_details', $transferDetails);

                $currentBranchInventoryCount = $this->db->query("select * from tbl_product_inventory where product_id = ? and branch_id = ?", [$cartProduct->product_id, $this->session->userdata('BRANCHid')])->num_rows();
                if ($currentBranchInventoryCount == 0) {
                    ...
                    $this->db->insert('tbl_product_inventory', $currentBranchInventory);
                } else {
                    $this->db->query("
    ...
    public function updateProductTransfer()
    {
        ...
        try {
            ...
            $oldTransfer    =   $this->db->query("select * from tbl_transfer_master where transfer_id = ?", $transferId)->row();
            ...
            $this->db->where('transfer_id', $transferId)->update('tbl_transfer_master', $transfer);

            $oldTransferDetails = $this->db->query("select * from tbl_transfer_details where transfer_id = ?", $transferId)->result();
            ...
            $this->db->query("delete from tbl_transfer_details where transfer_id = ?", $transferId);
    ...
    public function getTransfers()
    {
        ...
        $transfers = $this->db->query("
                ...
                join tbl_outlet f on f.branch_id = tm.transfer_from
    ...
    public function deleteTransfer()
    {
        ...
        try {
            ...
            $oldTransfer = $this->db->query("select * from tbl_transfer_master where transfer_id = ?", $transferId)->row();
            $oldTransferDetails = $this->db->query("select *, p.Product_Code, p.Product_Name from tbl_transfer_details left join tbl_product p on p.Product_SlNo = product_id where transfer_id = ?", $transferId)->result();
            ...
            foreach ($oldTransferDetails as $oldDetails) {
                ...
                $this->db->update("tbl_transfer_details", ['status' => 'd', 'DeletedBy' => $this->session->userdata("userId"), 'DeletedTime' => date('Y-m-d H:i:s'), 'last_update_ip' => get_client_ip()]);
            }

            $this->db->where("transfer_id", $transferId);
            $this->db->update("tbl_transfer_master", ['status' => 'd', 'DeletedBy' => $this->session->userdata("userId"), 'DeletedTime' => date('Y-m-d H:i:s'), 'last_update_ip' => get_client_ip()]);
    ...
    public function receivedTransfer()
    {
        ...
        try {
            ...
            $transfer = $this->db->query("select * from tbl_transfer_master where transfer_id = ?", $data->transferId)->row();
            $transferDetails = $this->db->query("select * from tbl_transfer_details where transfer_id = ?", $data->transferId)->result();
            foreach ($transferDetails as $key => $cartProduct) {
                $transferToBranchInventoryCount = $this->db->query("select * from tbl_product_inventory where product_id = ? and branch_id = ?", [$cartProduct->product_id, $transfer->transfer_to])->num_rows();
                if ($transferToBranchInventoryCount == 0) {
                    ...
                    $this->db->insert('tbl_product_inventory', $transferToBranchInventory);
                } else {
                    $this->db->query("
                                update tbl_product_inventory
            ...
            $this->db->where("transfer_id", $data->transferId)->update('tbl_transfer_details', $rules);
            $rules['receivedBy'] = $this->session->userdata("userId");
            $this->db->where("transfer_id", $data->transferId)->update('tbl_transfer_master', $rules);
