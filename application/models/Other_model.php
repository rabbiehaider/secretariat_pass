...
class Other_model extends CI_Model
{
	...
	public function branch_wise_category(){
		$res = $this->db->where('branch_id', $this->BRANCHid)->where('status', 'a')->order_by('Category_SlNo', 'asc')->get('tbl_category')->result();
		if($res){
			return $res;
		}else{
			return FALSE;
	...
	public function get_single_category_info($id){
		$res = $this->db->where('Category_SlNo', $id)->where('branch_id', $this->BRANCHid)->where('status', 'a')->get('tbl_category')->row();
		if($res){
			return $res;
		}else{
			return FALSE;
	...
	public function transaction_account_all($type = Null){
		...
		$this->db->select('tbl_cash_transactions.*,tbl_account.*')->from('tbl_cash_transactions');
		$this->db->join('tbl_account', 'tbl_account.Acc_SlNo=tbl_cash_transactions.Acc_SlID','left');
		$this->db->where('tbl_cash_transactions.status', 'a');
		$this->db->where('tbl_cash_transactions.branch_id', $this->BRANCHid);
		$this->db->where("DATE_FORMAT(tbl_cash_transactions.Tr_date,'%Y-%m-%d') >=",$startdate);
		$this->db->where("DATE_FORMAT(tbl_cash_transactions.Tr_date,'%Y-%m-%d') <=",$enddate);
		if($type != 'A'){
			$this->db->where('tbl_cash_transactions.Tr_Type',$tr_type);
		}
		$res = $this->db->order_by('tbl_cash_transactions.Tr_SlNo', 'desc')->get()->result();
	...
	public  function transaction_by_account($type = Null, $account_id = Null){
		...
		$this->db->where('tbl_cash_transactions.status', 'a');
		$this->db->where('tbl_cash_transactions.branch_id', $this->BRANCHid);
		$this->db->where('tbl_cash_transactions.Acc_SlID', $account_id);
		$this->db->where("DATE_FORMAT(tbl_cash_transactions.Tr_date,'%Y-%m-%d') >=",$startdate);
		$this->db->where("DATE_FORMAT(tbl_cash_transactions.Tr_date,'%Y-%m-%d') <=",$enddate);
		if($type != 'A'){
			$this->db->where('tbl_cash_transactions.Tr_Type',$tr_type);
